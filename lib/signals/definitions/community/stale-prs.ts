...
export const stalePRs: SignalDefinition = {
  meta: {
    ...
    fixInfo: {
      ...
      objective: 'Each stale PR has received a follow-up comment appropriate to its context',
      prompt: [
        'You are a maintainer of {{repoFullName}}. The following PRs have gone stale without response:',
        '',
        '{{#each prs}}',
        '- PR #{{this.number}} "{{this.title}}" by @{{this.author}} — {{this.daysSinceUpdate}} days without update ({{#if this.isExternal}}external contributor{{else}}internal{{/if}})',
        '{{/each}}',
        '',
        'For each PR:',
        '1. Read the PR diff to understand what it does',
        '2. Write a thoughtful, specific follow-up comment. If the PR looks good, say so and indicate you will review soon. If it needs changes, be specific about what. If it is abandoned or superseded, suggest closing it.',
        '3. Post the comment',
        '',
        'Be genuine and respectful — these are real contributors waiting for a response.',
      ].join('\n'),
